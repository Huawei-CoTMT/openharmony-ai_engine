...
#ifndef DEMO_XXXX_PLUGIN_H
#define DEMO_XXXX_PLUGIN_H
...
namespace OHOS {
namespace AI {
struct DemoXxxxPluginConfig : PluginConfig {
    size_t maxOutputSize;
};

class DemoXxxxPlugin : public IPlugin {
public:
    DemoXxxxPlugin();
    virtual ~DemoXxxxPlugin();
...
private:
    int32_t BuildConfig(intptr_t handle, DemoXxxxPluginConfig &config);
    int32_t DoProcess(intptr_t handle, const DemoXxxxPluginConfig &config, IRequest *request, IResponse *&response);
    int32_t MakeInference(intptr_t handle, const DemoXxxxPluginConfig &config, DataInfo &outputInfo);
    void ReleaseAllHandles();

private:
    std::mutex mutex_;
    std::shared_ptr<EngineAdapter> adapter_;
    std::map<intptr_t, DemoXxxxPluginConfig> handles_;
};
} // namespace AI
} // namespace OHOS
#endif // DEMO_XXXX_PLUGIN_H
